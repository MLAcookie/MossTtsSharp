...
internal class CodecDecoderStreaming : IDisposable
{
    ...
    private static readonly int[] _contexts =
    [
        500,
        500,
        500,
    ...
    private readonly List<int> _codeBuffer = [];
    ...
    public CodecDecoderStreaming(string codecModelDir)
    {
        ...
        _session = new InferenceSession(
            Path.Combine(codecModelDir, "moss_audio_tokenizer_decode_step.onnx"),
            opts
        );
    ...
    {
        var frameCount = _codeBuffer.Count / Nvq;
        var flatCodes = _codeBuffer.ToArray();
        _codeBuffer.Clear();
        _batchesDecoded++;
        ...
        var audioLength = results[1].AsTensor<int>().ToArray()[0];
        ...
        for (var l = 0; l < 12; l++)
        {
            ...
            _cachedPositions[l] = CopyToBufferInt(
                _cachedPositions[l],
                results[outIdx++].AsTensor<int>()
            );
        }
        ...
        var flat = audio.ToArray();
        var interleaved = new float[audioLength * channels];

        for (var t = 0; t < audioLength; t++)
        {
    ...
    public void ResetCache()
    {
        ...
        {
            var ctx = _contexts[l];
            var size = 1 * NumHeads * ctx * HeadDim;
            if (_cachedKeys[l].Length != size)
                _cachedKeys[l] = new float[size];
            else
                Array.Clear(_cachedKeys[l]);
            if (_cachedValues[l].Length != size)
                _cachedValues[l] = new float[size];
            else
                Array.Clear(_cachedValues[l]);
            _attnOffsets[l] = 0;
            var posSize = 1 * ctx;
            if (_cachedPositions[l].Length != posSize)
                _cachedPositions[l] = new int[posSize];
            Array.Fill(_cachedPositions[l], -1);
        }
